from model.func_data import request_data, check_time, failure_intervals
from model.calc_gamma import probabilidade_de_falha, probabilidade_cenarios



sensor = 'energia'
urls = 'http://crdamke.cloud/extrator'

df = request_data(sensor, urls)
df = check_time(df, col_timestamp='timestamp')
lista_falhas = failure_intervals(df, status1='SEM_ENERGIA')
prob_falha = probabilidade_de_falha(lista_falhas[1], tempo_ate_falha=5)
print(prob_falha)
